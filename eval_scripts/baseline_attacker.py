...
BASE_OUTPUT_DIR = "privacy_model_baseline_attacker"
BATCH_SIZE = 8
TEST_SPLIT_SIZE = 0.1
NUM_RUNS = 3
SPLIT_SEED = 42

# Only filter for the original datasets
TARGET_DATASETS = [
    "trustpilot_10000_sample.csv",
    "enron_10000_sample.csv",
    "yelp_10000_sample.csv"
]
...
def prepare_dataset(df, dataset_name):
    ...
    df = df[[original_col, 'labels']].copy()
    df = df.rename(columns={original_col: 'original_text'})
...
def main():
    ...
    processed_datasets = []
    for result_file in ["baseline_attacker_yelp_results.csv", "baseline_attacker_trustpilot_results.csv", "baseline_attacker_enron_results.csv"]:
    ...
    dataset_paths = [os.path.join(DATASET_FOLDER, dataset) for dataset in TARGET_DATASETS]
    dataset_paths = [path for path in dataset_paths if os.path.exists(path)]
    ...
    for dataset_path in dataset_paths:
        ...
        if classification_type == 'author' and 'yelp' in dataset_name.lower():
            results_file = "baseline_attacker_yelp_results.csv"
        elif classification_type == 'gender':
            results_file = "baseline_attacker_trustpilot_results.csv"
        elif classification_type == 'author' and 'enron' in dataset_name.lower():
            results_file = "baseline_attacker_enron_results.csv"
        else:
            results_file = "baseline_attacker_results.csv"
        
        dataset = Dataset.from_pandas(df)
        
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
        tokenize_fn = lambda x: tokenizer(x["original_text"], truncation=True, padding="max_length", max_length=512)
