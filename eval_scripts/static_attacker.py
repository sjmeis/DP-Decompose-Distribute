...
DATASET_FOLDER = "privatized_datasets/enron/llr"
BASE_OUTPUT_DIR = "privacy_model_static_attacker"
BATCH_SIZE = 8
TEST_SPLIT_SIZE = 0.1
NUM_RUNS = 3
SPLIT_SEED = 42
...
def prepare_dataset(df, dataset_name):
    ...
    df = df[[original_col, privatized_col, 'labels']].copy()
    df = df.rename(columns={original_col: 'original_text', privatized_col: 'privatized_text'})
...
def main():
    ...
    processed_datasets = []
    for result_file in ["static_attacker_yelp_results.csv", "static_attacker_trustpilot_results.csv", "static_attacker_enron_results.csv"]:
    ...
    for dataset_path in dataset_paths:
        ...
        if classification_type == 'author' and 'author_id' in pd.read_csv(dataset_path).columns:
            results_file = "static_attacker_yelp_results.csv"
        elif classification_type == 'gender':
            results_file = "static_attacker_trustpilot_results.csv"
        elif classification_type == 'author':
            results_file = "static_attacker_enron_results.csv"
        else:
            results_file = "static_attacker_results.csv"
        
        dataset = Dataset.from_pandas(df)
        
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
        tokenize_train_fn = lambda x: tokenizer(x["original_text"], truncation=True, padding="max_length", max_length=512)
        tokenize_test_fn = lambda x: tokenizer(x["privatized_text"], truncation=True, padding="max_length", max_length=512)
        ...
        tokenized_test = test_dataset.map(tokenize_test_fn, batched=True)
        ...
        for i in range(NUM_RUNS):
            ...
            tokenized_train = shuffled_train.map(tokenize_train_fn, batched=True, remove_columns=cols_to_remove_train)
            ...
            for class_idx in range(num_labels):
                class_label = id2label[class_idx]
                print(f"Class {class_idx} ({class_label}): F1={eval_results[f'eval_f1_class_{class_idx}']:.4f}, "
                      f"Precision={eval_results[f'eval_precision_class_{class_idx}']:.4f}, "
                      f"Recall={eval_results[f'eval_recall_class_{class_idx}']:.4f}")
